...
Citizen.CreateThread(function()
    while not isLoaded do
        Citizen.Wait(1)
        local interior = GetInteriorAtCoords(-308.88, 777.37, 118.77) -- Valentine Bank(val_bank_int_main)
        ...
        local interior = GetInteriorAtCoords(-310.0119, 802.9316, 117.9846) -- Valentine Saloon (val_saloon_int)
        local isValid = IsValidInterior(interior)
        if isValid then
        ...
        local interior = GetInteriorAtCoords(-273.4513, 811.3408, 118.38) -- Valentine Jail (val_jail_int)
        local isValid = IsValidInterior(interior)
        if isValid then
        ...
        local interior = GetInteriorAtCoords(-323.53, 801.72, 116.86) -- Valentine General Store (val_genstore)
        local isValid = IsValidInterior(interior)
        if isValid then
            if not IsInteriorEntitySetActive(interior, "_p_apple01x_dressing") then
        ...
        local interior = GetInteriorAtCoords(-782.50, -1324.0, 44.0) -- Blackwater General Store (bla_genstore_int)
        local isValid = IsValidInterior(interior)
        if isValid then
            if not IsInteriorEntitySetActive(interior, "_p_apple01x_dressing") then
                ActivateInteriorEntitySet(interior, "_p_apple01x_dressing")
                ...
                ActivateInteriorEntitySet(interior, "_s_canApricots01x_dressing")
                ActivateInteriorEntitySet(interior, "_s_canApricots01x_group")
                ActivateInteriorEntitySet(interior, "_s_candyBag01x_red_group")
                ActivateInteriorEntitySet(interior, "_s_canKidney01x_dressing")
                ActivateInteriorEntitySet(interior, "_s_cankidney01x_group")
        ...
        local interior = GetInteriorAtCoords(-3686.06, -2626.68, -15.12) -- Armadillo General Store (arm_genstore_int)
        local isValid = IsValidInterior(interior)
        if isValid then
            if not IsInteriorEntitySetActive(interior, "_p_cigar02x_dressing") then
                ActivateInteriorEntitySet(interior, "_p_cigar02x_dressing")
                ActivateInteriorEntitySet(interior, "_p_cigar02x_group")
                ActivateInteriorEntitySet(interior, "_p_tin_pomade01x_dressing")
                ActivateInteriorEntitySet(interior, "_p_tin_pomade01x_group")
        ...
        local interior = GetInteriorAtCoords(3024.03, 561.68, 43.01) -- Van Horn Exotic Goods (van_exoticgoods_int)
        local isValid = IsValidInterior(interior)
        if isValid then
            if not IsInteriorEntitySetActive(interior, "_p_cigarettebox01x_dressing") then
                ActivateInteriorEntitySet(interior, "_p_cigarettebox01x_dressing")
                ActivateInteriorEntitySet(interior, "_p_cigarettebox01x_group")
                ActivateInteriorEntitySet(interior, "_p_package01x_dressing")
                ActivateInteriorEntitySet(interior, "_p_package01x_group")
        ...
        local interior = GetInteriorAtCoords(-398.193, 1725.221, 216.431) -- Chez Porter Cabin (che_cabin1)
        local isValid = IsValidInterior(interior)
        if isValid then
        ...
        local interior = GetInteriorAtCoords(vec3(-5489.375, -2939.493, -0.402)) -- Tumbleweed General Store (tum_genstore_int)
        local isValid = IsValidInterior(interior)
        if isValid then
        ...
        local interior = GetInteriorAtCoords(vec3(1326.34, -1291.64, 76.99)) -- Rhodes General Store (rho_genstore_int)
        local isValid = IsValidInterior(interior)
        if isValid then
        ...
        local interior = GetInteriorAtCoords(vec3(1625.681, -364.560, 75.897)) -- Lonnie's Shack (lnn_lonnie_shack_int)
        local isValid = IsValidInterior(interior)
        if isValid then
        ...
        local interior = GetInteriorAtCoords(vec3(-285.066, 806.314, 119.386)) -- Valentine Doctor (val_doctor_int)
        local isValid = IsValidInterior(interior)
        if isValid then
        ...
        local interior = GetInteriorAtCoords(vec3(2714.09, -1285.94, 49.65)) -- Saint Denis Gun Shop (new_gunshop_int)
        local isValid = IsValidInterior(interior)
        if isValid then
            if not IsInteriorEntitySetActive(interior, "l_00003ab1jo") then
                ActivateInteriorEntitySet(interior, "_s_inv_arrowammo01x_dressing")
                ...
                ActivateInteriorEntitySet(interior, "_s_inv_varmint_rifleammo01x_group")
                ActivateInteriorEntitySet(interior, "l_00003ab1jo")
                ActivateInteriorEntitySet(interior, "l_0032bd22vj")
                ActivateInteriorEntitySet(interior, "l_0078655bjv")
                ActivateInteriorEntitySet(interior, "l_012cc3a9aj")
            end
        end

        local interior = GetInteriorAtCoords(vec3(2948.08, 1318.51, 43.12)) -- Annesburg Gun Shop (ann_gunsmith_int)
        local isValid = IsValidInterior(interior)
        if isValid then
            if not IsInteriorEntitySetActive(interior, "_sign_pistolAmmo_dressing") then
